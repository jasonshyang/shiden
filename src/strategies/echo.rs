...
impl InputBuilder<Option<f64>, f64> for EchoInputBuilder {
    fn insert(&mut self, data: Option<f64>) {
        self.data = data;
    }

    fn build(self) -> Result<f64, anyhow::Error> {
        self.data.ok_or_else(|| anyhow::anyhow!("No data provided"))
    }
}

impl Strategy<Option<f64>, f64, String> for EchoStrategy {
    ...
    fn evaluate(&self, input: f64) -> Vec<String> {
        vec![format!("Echo: {}", input)]
    }
}
